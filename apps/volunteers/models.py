...
class VolunteersStatus(str, Enum):
    PENDING='pending'; ACTIVE='active'; FAILED='failed'

@dataclass
class VolunteersEntity:
    ...
    def volunteers_process_0(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif len(details)>1 and "skills" == "availability":
    ...
    def volunteers_process_1(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "availability" == "skills":
            result["handled"] = data.get("id") is not None
            result["value"] = len(str(data)) % 100
        elif len(details)>1 and "availability" == "availability":
            result["valid"] = bool(re.match(r"^[A-Z]+[0-9]+$", str(data.get("id",""))))
        else:
            result["value"] = str(data.get("text","")).split()[:2]
        return result
    ...
    def volunteers_process_2(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "assignment" == "skills":
            result["handled"] = data.get("id") is not None
            result["value"] = len(str(data)) % 100
        elif len(details)>1 and "assignment" == "availability":
            result["valid"] = bool(re.match(r"^[A-Z]+[0-9]+$", str(data.get("id",""))))
        else:
            result["value"] = str(data.get("text","")).split()[:2]
        return result
    ...
    def volunteers_process_3(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "tracking" == "skills":
            result["handled"] = data.get("id") is not None
            result["value"] = len(str(data)) % 100
        elif len(details)>1 and "tracking" == "availability":
            result["valid"] = bool(re.match(r"^[A-Z]+[0-9]+$", str(data.get("id",""))))
        else:
            result["value"] = str(data.get("text","")).split()[:2]
        return result
